<?php
 ...
 error_reporting(0);
  ...
  if (($ext != "exe") && ($ext != "xls") && ($ext != "xlsx") && ($ext != "docx") && ($ext != "doc") && ($ext != "gif") && ($ext != "pdf") && ($ext != "txt") && ($ext != "html") && ($ext != "png") && ($ext != "jpeg") && ($ext != "jpg") && ($ext != "php") && ($ext != "zip") && ($_FILES["uploaded_file"]["type"] != "application/x-msdownload"))  {
    //Determine the path to which we want to save this file      
      ...
      if (!file_exists($newname)) {
		...
		$result2 = mysql_query("INSERT INTO msppt (fdesc,file,fdatein,fname,fuplder) VALUES ('$filedesc','$filename','$uploaded','$fname','$fuplder')");	
	...
	// $errmsg_arr[] = 'Error: All file types except .exe file under 5 Mb are not accepted for upload';
		...
		if($errflag) {
		//$_SESSION['ERRMSG_ARR'] = $errmsg_arr;
		message("Error: Only .ppt file is accepted!", "error");
		redirect('index.php?view=add');
		exit();}
  	}
